...
export const serveMode: Mode = {
  ...
  async run() {
    ...
    lines.push(`  ${theme.accent("●")} POST /api/v1/chat       ${theme.dim("(send messages, supports ?stream=true)")}`)
    lines.push(`  ${theme.accent("●")} GET  /api/v1/health`)
    lines.push(`  ${theme.accent("●")} GET  /api/v1/chat/providers`)
    lines.push(`  ${theme.accent("●")} GET  /api/v1/agents`)
    lines.push(`  ${theme.accent("●")} GET  /api/v1/metrics`)
    lines.push("")

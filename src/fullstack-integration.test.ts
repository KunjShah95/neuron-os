...
describe("Fullstack Integration Tests", () => {
  ...
  function collectWsMessages(ws: WebSocket, predicate: (msg: any) => boolean, timeoutMs = 15_000): Promise<any[]> {
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        ...
        reject(
          new Error(
            `Timed out waiting for predicate. Collected ${messages.length} messages: ${JSON.stringify(
              messages.map((m: any) => m.event),
            )}`,
          ),
        )
      }, timeoutMs)
  ...
  async function cleanup(server: { stop: () => void }, agentIds: string[]) {
    for (const id of agentIds) {
      try {
        await agentManager.kill(id, 2_000)
      } catch {
        /* best effort */
      }
    }
  ...
  it("should full stack ws event flow", async () => {
    ...
    try {
      ...
      const spawnPromise = collectWsMessages(ws, (msg) => msg.event === "agent:ready", 15_000)
